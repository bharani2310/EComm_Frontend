...
const SearchBar = () => {
  ...
  const handleItemClick = (item) => {
    ...
    if (item.type === "product") {
      navigate(`/product/${item.id}`, { state: { item } }); // Pass state
    } else if (item.type === "category") {
      navigate(`/details/${item.id}`, { state: { item } }); // Pass state
    }
  ...
  return (
    <div className="relative w-full max-w-md" ref={searchRef}>
      ...
      {suggestions.length > 0 && (
        <ul className="absolute bg-white border border-gray-200 w-full mt-1 rounded shadow-lg">
